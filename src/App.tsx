...
// --- fresh engine helper (no reusing disposed workers) ---
async function freshEngine(): Promise<EngineWrapper> {
...
export default function App() {
  ...
  const handleStartGame = async (side: "w" | "b", level: number) => {
    ...
    // kill old engine if exists
    if (engineRef.current) {
      engineRef.current.terminate();
    }
    // start new engine
    engineRef.current = await freshEngine();
    ...
    // if AI is black and must respond first
    if (side === "b") {
      aiMove();
    }
  };
